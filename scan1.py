...
def transform(hsv): 
    ...
    tmp2 = 1 - (1 - hsv[1]) ** .75
    return np.array([
        tmp2 * np.cos(2 * np.pi * tmp1), tmp2 * np.sin(2 * np.pi * tmp1), hsv[2] * .5
    ])
...
def distances(points1, points2):
    res = np.zeros((points1.shape[0], points2.shape[0]))
    for i in range(points1.shape[0]):
        res[i] = np.array([dist(points1[i], p) for p in points2])
    return res
...
Assignment = namedtuple('Assignment', ['conf', 'facelet', 'col'])


class ColorMatcher:

   def match(self, bgrs, debug=True):
        ...
        distm1 = distances(transformed, centers)
        order = np.argsort(distm1, axis=1)
...
if __name__ == '__main__':
    ...
    matcher.match(scans, debug=True)
